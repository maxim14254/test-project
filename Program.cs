...
namespace ConsoleApp1
{
    class Program
    {
        ...
        { 
            if (i == 0)//выполняется 1 раз 
            {
                bool b = true;//переменная открывающая проход для добавление артиста 
                ...
                while (reader.Read())
                {
                    int а = String.Compare(group, reader.GetValue(0).ToString());// сравниваем артиста которого ввели и который находится в БД
                    if (а==0)//если они совпали, то закрываем проход на добавление нового артиста в БД
                    {
                        b = false;
                        break;
                    }
                ...
                if(b)// проход на добавление артиста в БД
                {
